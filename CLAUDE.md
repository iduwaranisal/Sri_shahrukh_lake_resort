...
argument-hint: "[<target>] [--url URL] [--lang ja|en] [--viewport mobile-sm|mobile|tablet|tablet-pro|desktop|desktop-lg|WxH] [--electron-app PATH] [--scenario-mode locked|refresh|hybrid] [--only categories] [--skip categories] [--yes|-y]"
user-invocable: true
...
## Must do task
Audit after every code changes, it is the primary requirement of the development. Always Audit the ux using uxaudit skill after every code changes.Audit after every code changes it is a must do task. need to audit ux using this skill guidelines after every code changes, every change in the ui or backend ux must be thoroughly audited it is the primary requirement.
